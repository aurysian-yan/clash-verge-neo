...
> [!IMPORTANT]
> 关于版本的说明：Clash Verge Tahoe 版本号遵循 x.y.z：x 为重大架构变更，y 为功能新增，z 为 Bug 修复。
...
- 修复将快捷键名称更名为 `Clash Verge Tahoe`之后无法删除图标和无法删除注册表
...
- **前端与界面体验：**
  ...
  - Windows 快捷键名称更名为 `Clash Verge Tahoe`
  - 更新失败可回退至使用代理重试
...
- Linux 下 Clash Verge Tahoe Rev 内存占用显著高于 Windows / MacOS
...
- Clash Verge Tahoe Rev 从现在开始不再强依赖系统服务和管理权限
...
代号释义： 本次发布在功能上的大幅扩展。新首页设计为用户带来全新交互体验，DNS 覆写功能增强网络控制能力，解锁测试页面助力内容访问自由度提升，轻量模式提供灵活使用选择。此外，macOS 应用菜单集成、sidecar 模式、诊断信息导出等新特性进一步丰富了软件的适用场景。这些新增功能显著拓宽了 Clash Verge Tahoe 的功能边界，为用户提供了更强大的工具和可能性。
...
- 检测所需的Clash Verge Tahoe Service版本（杀毒软件误报可能与此有关，因为检测和安装新版本Service需管理员权限）
...
- 历时3个月的紧密开发与严格测试稳定版2.0.0终于发布了：巨量改进与性能、稳定性提升，目前Clash Verge Tahoe Rev已经有了比肩cfw的健壮性；而且更强大易用！
...
- Clash Verge Tahoe Rev 目前已进入稳定周期，日后更新将着重于 bug 修复与内核常规升级
